...
notifications through your Mailgun account, **no additional work is needed**.
...
The `overrides` parameter accepts all of the [Mailgun's Messages API](https://documentation.mailgun.com/en/latest/api-sending.html#sending)
body parameters.
...
"notification": {
  ...
  "overrides": {
    "providers": {
      "mailgun": {
        "o:tag": "onboarding",
        "h:X-Original-To": "team@example.com"
      }
    }
  }
}
...
Create a template in Mailgun, and grab its ID. Then create a notification with
the `template` attribute set in `overrides.providers.postmark`. MagicBell will
use `custom_attributes` to fill in your template.
