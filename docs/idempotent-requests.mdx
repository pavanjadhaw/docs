...
If you attempt an operation twice or more, only the first attempt will be processed.
For example, if a request to create a notification does not respond due to a network connection error,
you can retry the request with the same idempotency key, and no additional notification will be created.

To perform an idempotent request, you need to add a `Idempotency-Key: <key>` header, on the request.
...
idempotency key will return the same result. The request results are cached for 24 hours.
